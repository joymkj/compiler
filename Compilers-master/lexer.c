...
void printToken(TokenInfo tk){
    if(tk==NULL) printf(RED"Empty Token!\n"RESET);
    else printf("Token no. %2d, LineNo %2d, Lexeme: %s\n",-1*(tk->token),tk->lineNo,tk->lexeme);
}
...
LexBuf newBuf(char* fname,int size,RevLookupHashTable ht){
    ...
    if(f==NULL){
        perror("fopen");
        printf("File"RED" %s "RESET"does not exist!\n"
			"Quitting because of file read error.\n",fname);
        exit(0);
    }
    ...
    b->ptr = 0;
    int nread = (int)fread(b->buf+b->ptr,sizeof(char),b->size,b->fp);
    b->end = nread;
    b->ht = ht;
    return b;
}
...
TokenInfo getToken(LexBuf b){
    ...
    int state = 0, flag=0;
    char* lexName  = NULL; //lexName can only be 30
    int lexLen = 0;
    char c;// = fgetc(b);
    while(flag==0){
        c = bgetc(b);
        ...
        switch(state){
            ...
            case 15:
                if(lexLen>=29){
                    ...
                    else return makeTok(lexName,tok,lineNo);
                }            
...
void removeComments(char *testcaseFile, char *cleanFile){
    int bufSize = 512;// sysconf(_SC_PAGESIZE);
    char buf[bufSize];
    int i;
